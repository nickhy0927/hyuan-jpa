...
public class QuartzManager {
	...
	public static void addJob(String jobName, Class<?> clazz, String cron) {
		try {
			...
			// 任务名，任务组，任务执行类
			JobDetail jobDetail = JobBuilder.newJob((Class<? extends Job>) clazz).withIdentity(jobName, JOB_GROUP_NAME).build();
			// 触发器
			TriggerBuilder<Trigger> triggerBuilder = TriggerBuilder.newTrigger();
			// 触发器名,触发器组
			triggerBuilder.withIdentity(clazz.getSimpleName(), TRIGGER_GROUP_NAME);
