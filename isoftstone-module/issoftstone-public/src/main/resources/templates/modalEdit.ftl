		...
		$(function () {
			layui.use(['form'], function () {
                ...
                form.on('submit(edit-form)', function (data) {
                	$.saveInfo({
                		url: '${r'${ctx}'}/${pathSuffix}/${domainObjectName?lower_case}/${domainObjectName?lower_case}EditUpdate.json',//发送请求
				    	data: $('form').getForm(),
                ...
                $.ajax({
			    	url: '${r'${ctx}'}/${pathSuffix}/${domainObjectName?lower_case}/${domainObjectName?lower_case}EditJson.json',//发送请求
			    	data: {id : '${r'${id}'}'},
